...
function preventBrowserCaching() {
  ...
  window.fetch = function(...args) {
    ...
    if (typeof url === 'string') {
      // Skip cache-busting for external APIs (Google, OpenAI, etc.)
      if (url.includes('googleapis.com') || url.includes('openai.com') || url.includes('huggingface.co')) {
...
function setupAuthentication() {
  ...
  function showUserMenuWithElements(user, authButtonsEl, userMenuEl) {
    ...
    if (userAvatar) {
      ...
      if (avatarUrl) {
        // DISABLED: Google profile photos need specific URL parameters to work correctly
        // let processedUrl = avatarUrl;
        console.log('� Original photoURL:', avatarUrl);
        
        // DISABLED: If it's a Google profile photo, ensure we have the right size parameter
        ...
        // Enhanced Google photo URL processing
        let processedUrl = avatarUrl;
      ...
      userAvatar.onerror = () => {
        console.warn('❌ Failed to load avatar, trying fallback options');
        
        // If we tried a processed URL and it failed, try the original
        if (avatarUrl && userAvatar.src !== avatarUrl && avatarUrl.includes('googleusercontent.com')) {
          console.log('🔄 Processed URL failed, trying original Google URL:', avatarUrl);
        ...
        const fallbackUrl = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName || user.email)}&background=667eea&color=fff&size=64&rounded=true`;
        userAvatar.src = fallbackUrl;
        console.log('🎨 Using generated avatar fallback:', fallbackUrl);
        
        // Final fallback if even generated avatar fails
        userAvatar.onerror = () => {
          console.warn('❌ Generated avatar failed, using SVG placeholder');
          userAvatar.src = 'data:image/svg+xml;base64,';
        };
      };
...
// Your API keys (hardcoded for all users)
const API_KEYS = {
  googlestudio: 'AIzaSyAlIVaCVDjT0jib-MdsaAgmGMX7wo4DVIM',
  huggingface: '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'
};
...
function finalizeStreamingMessage(messageElement, finalText) {
  ...
  if (content) {
    ...
    // Re-apply syntax highlighting and math rendering
    if (window.Prism) {
